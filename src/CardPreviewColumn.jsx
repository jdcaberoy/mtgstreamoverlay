              ...
              <div style={{ maxHeight:100, overflowY:'auto' }}>
                {parsed.map((sec, si) => (
                  <div key={si}>
                    ...
                    {sec.cards.map((c, ci) => (
                      <div key={ci} style={{ fontSize:11, color:'#444', padding:'1px 0' }}><span style={{ color:'#888', fontWeight:700, marginRight:6 }}>{c.qty}×</span>{c.name}</div>
                    ))}
                  </div>
                ))}
              </div>
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center' }}
      ...
      <div style={{ background:'#fff', borderRadius:12, width:380, maxWidth:'95vw', boxShadow:'0 8px 40px rgba(0,0,0,0.25)', padding:20 }}>
        ...
        <div style={{ marginBottom:10 }}>
          <div style={fLabel}>Card Name</div>
          <input style={inp} value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Lightning Bolt" autoFocus />
        </div>
...
export default function CardPreviewColumn({ state, setState }) {
  ...
  // Secondary column uses the SAME shared card state (cardImage/cardName) and decks
  // but has its own dedicated overlay button + chroma picker
  ...
  function clearCard() { setState(p => ({ ...p, cardImage: null, cardName: '' })); }
  ...
  return (
    <>
      ...
      <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
        ...
        {/* Chroma picker for card overlay */}
        <ChromaPicker
        ...
        {state.cardImage && (
          <div style={{ position:'relative', marginBottom:10 }}>
            ...
            <button onClick={clearCard} style={{ position:'absolute', top:4, right:4, background:'rgba(0,0,0,0.5)', border:'none', color:'#fff', borderRadius:4, padding:'2px 7px', cursor:'pointer', fontSize:11 }}>✕</button>
          </div>
        )}
        ...
        <div style={{ border:'1px solid #ddd', borderRadius:'0 6px 6px 6px', flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 10px', borderBottom:'1px solid #eee', background:'#fafafa', flexWrap:'wrap' }}>
            ...
            <button onClick={() => setShowAddCard(true)} style={toolBtn}>+ Card</button>
            <button onClick={() => setShowImport(true)} style={{ ...toolBtn, background:'#1a73e8', color:'#fff', borderColor:'#1a73e8' }}>↩ Import</button>
          </div>
